...
run_model = 'Obstacle_run'
current_path = '/home/pi/Top_WRO2025'
sys.path.append(current_path)

model_name_occw = "/CCWORM/mypilot.h5"
model_name_ocw = "/CWORM/mypilot.h5"
model_name = model_name_occw
...
def read_gz():
    lo, hi = bus.read_i2c_block_data(0x50, 0x39, 2)
    val = (hi << 8) | lo
    if val > 32767:
        val -= 65536
    return (val / 32768.0 * 2000.0)*-1*gyro_offset
...
#
def read_now_yaw():
    global bias,angle,last_t


    now = time.time()
    rate = read_gz() - bias
    angle += rate * (now - last_t)
    last_t = now
    return angle



def ccwstart_from_parkinglot():
    final_gyro_degree = read_now_yaw()
...
def cwstart_from_parkinglot():
    ...
    pwm.set_pwm(0, 0, servo_signal)
    pwm.set_pwm(1, 0, servo_steering_signal)
    while abs(final_gyro_degree) < 8:
            final_gyro_degree = read_now_yaw()
            print(final_gyro_degree)
            time.sleep(0.1)
    ...
    servo_signal = default_servo_signal- 27
    pwm.set_pwm(0, 0, servo_signal)
    while abs(final_gyro_degree) < 20:
        final_gyro_degree = read_now_yaw()
        print(final_gyro_degree)
        time.sleep(0.1)
    servo_signal = default_servo_signal
    ...
    pwm.set_pwm(0, 0, servo_signal)
    pwm.set_pwm(1, 0, servo_steering_signal)
    while abs(final_gyro_degree) < 35:
        final_gyro_degree = read_now_yaw()
        print(final_gyro_degree)
        time.sleep(0.1)
...
#     previous_time = time.time()
    ...
    if model_name == model_name_occw:
        ccwstart_from_parkinglot()
    else:
        cwstart_from_parkinglot()
    ...
    while Run_main:
        if time.time()-t2<1.5:
            ...
            prooffset = 0
        final_gyro_degree = round(read_now_yaw(),2)#final_gyro_degree = read_cumulative_yaw()
...
#         frame = cv2.cvtColor(frame,cv2.COLOR_RGB2BGR)
        ...
        if servo_signal > default_servo_signal+ default_servo_signal_offset+ prooffset:
            servo_signal = int(default_servo_signal+ default_servo_signal_offset+prooffset)
        if servo_signal < default_servo_signal- (default_servo_signal_offset+prooffset):
            servo_signal = int(default_servo_signal- (default_servo_signal_offset+prooffset))
